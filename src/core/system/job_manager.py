...
class JobManager:
    ...
    def _create_job_details(self, job_type: JobType, **kwargs: Any) -> dict:
        ...
        Args:
            ...
            **kwargs: Parameters specific to job type:
                     - For CRAWL jobs:
                       - source_id (UUID, optional): ID of the source to crawl.
    ...
    async def create_job(self, job_type: JobType, **kwargs: Any) -> Job:
        ...
        Example:
            # Auto-generate source_id
            job = await job_manager.create_job(job_type=JobType.CRAWL)

            # Use specific source_id
    ...
    async def update_job(self, job_id: UUID, updates: dict[str, Any]) -> Job:
        ...
        # Apply updates and validate new state
