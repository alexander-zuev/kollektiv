...
from src.models.chat_models import ContentBlock, ConversationMessage, ConversationSummary, Role, ToolUseBlock
...
class UserMessage(BaseModel):
    ...
    role: Role = Field(Role.USER, description="Role of the message sender")
    content: list[ContentBlock] = Field(..., description="Content of the message")
...
class MessageAcceptedEvent(BaseModel):
    ...
    new_conversation: bool = Field(
        ..., description="True if this is a new conversation, False if it is a continuation of an existing conversation"
    )
...
class MessageDoneEvent(BaseModel):
    ...
    message_id: UUID = Field(..., description="UUID of the message")
    streaming_complete: bool = Field(..., description="True if the streaming is complete, False if it is not")
...
class ChatResponse(BaseModel):
    ...
    event: MessageAcceptedEvent | MessageDeltaEvent | AssistantResponseEvent | ToolUseEvent | ErrorEvent = Field(
...
class ConversationListResponse(BaseModel):
    ...
    today: list[ConversationSummary] = Field(..., description="List of conversations from today")
    previous_7_days: list[ConversationSummary] = Field(
        ..., description="List of conversations from the previous 7 days"
    )
    previous_30_days: list[ConversationSummary] = Field(
        ..., description="List of conversations from the previous 30 days"
    )
