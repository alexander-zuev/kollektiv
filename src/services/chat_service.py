        ...
        try:
            ...
            yield ChatResponse(
                event=MessageAcceptedEvent(
                    ...
                    conversation_title="New conversation",  # TODO: refactor to be generated by an LLM
                )
            )
            logger.debug(f"Sent MessageAcceptedEvent for conversation: {conversation_history.conversation_id}")

            # 3. Add user message to pending
            await self.conversation_manager.add_pending_message(message=user_message)
            logger.debug(
                f"Added user message to pending for conversation {conversation_history.conversation_id}: "
                f"{user_message.content[:100]}..."
            )

            # 4. Process stream
            async for event in self._process_stream(conversation_history):
                yield event

            logger.info(f"Completed message processing for conversation {conversation_history.conversation_id}")
